                                    ...
                                    <table class="table table-bordered text-nowrap border-bottom" id="basic-datatable">
                                        ...
                                        @foreach($courierData as $courier)
                                            <tr>
                                                ...
                                                <td class="d-flex">
                                                    <a href="{{route('courier.edit',$courier->id)}}" class="btn btn-primary"><i class="fa fa-edit"></i></a>
                                                    <form action="{{route('courier.destroy',$courier->id)}}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-sm ms-2 btn-danger"><i class="fa fa-trash"></i></button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
